...
OpenpuxClient.prototype.rpc = function(ticket, args, method, params, callback)
{
    ...
    var url = this.buildRawUrl(ticket.scheme, ticket.host, args.url);
...
//          NewAccountTicketID: newAccountTicketID
//      }
//  };
//
OpenpuxClient.prototype.addAccount = function(ticket, args, callback)
{
    var self = this;
    ...
    obj.NewAccountID = args.items.NewAccountID;
    obj.NewAccountTicketID = args.items.NewAccountTicketID;
...
//
// OpenpuxData is a simple interface for clients to access
// the latest readings, and to post readings based on a tokenid.
//
...
// };
//
OpenpuxData.prototype.getLatestReading = function(callback) {
    ...
    var args = {
        AccountID: self.defaultAccount,
        SensorID: self.defaultSensorID,
        readingcount: 1,
        startdate: startdate,
        enddate: enddate
    };

    self.opclient.querySensorReadings(self.defaultTicket, args, function(error, response) {
        ...
        if (response.status != 200) {
            if (response.error != null) {
                callback(response.status + " response.error=" + response.error, null);
            }
            else {
                callback(response.status + " Error", null);
            }
            return;
        }

        if (typeof response.items == "undefined") {
            //console.log("200 OK, no readings returned");
            callback(null, null);
            return;
        }
...
if (typeof window != "undefined") {
    ...
    var erpc = function(method, params, successHandler, errorHandler, timeout)
    {
        ...
        if (g_erpcClient == null) {
            ...
            //
            // A null token is used since no authentication on this test
            // app is used.
            //
            // A null for host_args will load the origin from window.location to
            // communicate back to the server for the page.
            //
            g_erpcClient.defaultTicket = g_erpcClient.createTicket(null, "/", null);
        }
        ...
        g_erpcClient.rpc(g_erpcClient.defaultTicket, args, method, params, function(error, result) {
